...
CC_SRC_PATH="github.com/chaincode/chaincode_antonio02/go/"
if [ "$LANGUAGE" = "node" ]; then
        CC_SRC_PATH="/opt/gopath/src/github.com/chaincode/chaincode_antonio02/node/"
fi
...
addCapabilityToChannel() {
        ...
        if [ $CH_NAME != "testchainid" ] ; then
                if [ $GROUP == "orderer" ]; then
                      ...
                      # Sign with Peermicrosoft.Admin
                      signConfigtxAsPeerOrg 1 config_update_in_envelope.pb
                      # Sign with Peerapple.Admin
                      ...
                      # Sign with Peermicrosoft.Admin
                      signConfigtxAsPeerOrg 1 config_update_in_envelope.pb
                      # Prepare to sign the update as the Peerapple.Admin
                      setGlobals 0 2
                fi
        else
        ...
        if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
                peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.antonio.com:7050 --cafile $ORDERER_CA
                res=$?
                set +x
        else
                set -x
                peer channel update -f config_update_in_envelope.pb -c $CH_NAME -o orderer.antonio.com:7050 --tls true --cafile $ORDERER_CA
                res=$?
                set +x
        fi
...
#Query on chaincode on Peer0/microsoft
echo "Querying chaincode on microsoft/peer0..."
chaincodeQuery 0 1 90

#Invoke on chaincode on Peer0/microsoft
echo "Sending invoke transaction on microsoft/peer0..."
chaincodeInvoke 0 1
...
##Invoke on chaincode on Peer0/apple
echo "Sending invoke transaction on apple/peer0..."
chaincodeInvoke 0 2

sleep $DELAY

#Query on chaincode on Peer0/apple
echo "Querying chaincode on apple/peer0..."
