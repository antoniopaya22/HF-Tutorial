...
function upgradeNetwork () {
  docker inspect  -f '{{.Config.Volumes}}' orderer.antonio.com |grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer.antonio.com
  docker cp -a orderer.antonio.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.antonio.com
  docker-compose $COMPOSE_FILES up -d --no-deps orderer.antonio.com

  for PEER in peer0.microsoft.antonio.com peer1.microsoft.antonio.com peer0.apple.antonio.com peer1.apple.antonio.com; do
...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove the docker-compose yaml file that was customized to the antonio
...
# and two Peer Orgs (``microsoft`` & ``Apple``) each managing and maintaining two peer nodes.
...
# (``peer0.microsoft.antonio.com`` & ``peer0.Apple.antonio.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/MicrosoftMSPanchors.tx -channelID $CHANNEL_NAME -asOrg MicrosoftMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  ./channel-artifacts/AppleMSPanchors.tx -channelID $CHANNEL_NAME -asOrg AppleMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for AppleMSP..."
    exit 1
  fi
  echo
}
